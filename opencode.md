...
| `ptt_enabled`, `tts_enabled` | Startup defaults for the runtime `state` dict. **`tts_enabled` defaults to `false`** — spoken replies start off; `/tts` toggles them on |
...
- **Keyboard** — `_keyboard_input_loop` reads stdin via `_read_stdin_message`; handles
  slash commands. Multi-line **pastes arrive as one message**, not one turn per line:
  `import readline` at the top enables bracketed paste (the reliable path — the terminal
  wraps the paste in `ESC[200~`/`ESC[201~` and readline returns it whole, even when the
  last line has no newline), and `_read_stdin_message` additionally coalesces any line
  already queued within `_PASTE_IDLE_SECS` — the fallback for pastes made while the
  assistant is mid-reply, when bracketed paste is off. Long input is truncated in the
  console echo by `_echo_preview`; the full text still goes to Claude.
...
**Time awareness** — every persisted turn carries a local ISO-8601 `ts` (`_now_ts`).
The `ts` is a local-only key: `_with_time_context` builds the API payload by stripping it
and instead prefixing each timestamped **user** turn with
`[time: Wed 2026-07-29 20:14:03 UTC+02:00 | 4 hours since the previous message]`.
`chat_and_speak` calls it once when assembling `working_messages`, so tool-loop and
auto-continue turns stay header-free. Gaps are measured against the previous timestamped
turn (usually the assistant's reply). Pre-existing history entries have no `ts` and simply
get no header — no migration needed. `system_prompt.txt` tells Claude the header is
metadata and must never be read aloud.
